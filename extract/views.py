    ...
    if request.method == "POST" and request.FILES["document"]:
        ...
        try:
            ...
            extracted_data = ExtractedData.objects.create(
                document=document, extracted_text=text, document_type="image"
            )
            print("extracted_data", extracted_data)
            return render(
                request,
                "result.html",
                {"extracted_data": extracted_data, "image": "image"},
            )
    ...
    if request.method == "POST" and request.FILES["document"]:
        try:
            ...
            return render(
                request,
                "result.html",
                {"extracted_data": extracted_data, "pdf": "PDF", "pdf_url": pdf_url},
            )
        except Exception as e:
            error_message = f"Error processing image: {e}"
            return render(request, "error.html", {"error_message": error_message})
