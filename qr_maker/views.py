...
from .forms import MobileForm, QRForm, ZodiacForm
...
def cricket(request):
    ...
    )  # HTTP Get request to cricinfo rss feed
    ...
    def post(self, request):
        try:
            if "fetch-vid" in request.POST:
                ...
                title = "".join(
                    c if c.isalnum() or c in [" ", "_"] else "" for c in video.title
                )
                ...
                response = StreamingHttpResponse(
                    [video_data], content_type="application/octet-stream"
                )
