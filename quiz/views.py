...
from .models import Option, Question, Quiz, QuizTaker
    ...
    if last_quiz_taken_time_str:
        ...
        if time_elapsed < timedelta(days=1):
            ...
            return render(
                request,
                "too_soon.html",
                {"quiz": quiz, "time_remaining": time_remaining},
            )

    if request.method == "POST":
        score = calculate_score(request, questions)
        QuizTaker.objects.create(user=request.user, quiz=quiz, score=score)
...
def calculate_score(request, questions):
    ...
    for question in questions:
        ...
        try:
            correct_option_id = Option.objects.get(
...
def leaderboard(request):
    leaderboard_data = (
        QuizTaker.objects.values("user__username")
...
def user_scores(request, username):
    ...
    user_scores_data = QuizTaker.objects.filter(user=user)
    return render(request, "user_scores.html", {"user_scores": user_scores_data})
            ...
            for (
                ...
                for i, option_text in enumerate(options_text):
                    ...
                    Option.objects.create(
                        question=question, text=option_text, is_correct=is_correct
                    )
